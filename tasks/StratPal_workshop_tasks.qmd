...
1.  Define age-depth models for the different locations along the onshore-offshore gradient, and plot them. How do they connect to the chronostratigraphic diagram and the sea level curve shown in the introduction?
...
```{r}
p3(rate = 300, from = 0, to = 1) |> # on avg. 300 fossils per Myr.
...
gc = approxfun(scenarioA$t_myr, scenarioA$wd_m[,pos]) #  use linear interpolation between known water depths
```
...
#### Simulating and tansforming last occurences
...
```{r}
rolo = 100 # rate of last occurrences
