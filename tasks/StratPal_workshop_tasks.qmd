...
To find the documentation of a function, prepend its name with a question mark and execute it in the console (e.g., `?random_walk`). The help pages are also available on the package webpage ([mindthegap-erc.github.io/StratPal/](https://mindthegap-erc.github.io/StratPal/)) under "Reference".  
Long form documentation (vignettes) can be found on the webpage under "Articles" or via `browseVignettes("StratPal")`.

## Practical 1: Age-depth models and incompleteness (45 min)

This practical is an introduction to age-depth models and stratigraphic (in)completeness.
...
### Basin transect and chronostratigraphic diagram
...
```{r}
adm |>                                                            # take age-depth model
  get_hiat_duration() |>                                          # extract hiatus durations
  hist(xlab = paste("Hiatus duration", "[", get_T_unit(adm),"]"), # make histogram
       main = "Distribution of hiatus duration 2 km from shore")
```

Similarly, you can plot age-depth models using
...
seq(min_time(adm), max_time(adm), by = temp_res) |>           # time steps where the trait evolution is simulated
  ...
  plot(xlab = paste0("Stratigraphic position [",get_T_unit(adm),"]"), # plot
       ylab = "Trait value",
       type = "l",
       orientation = "lr") 
```
...
Note that we use the option `destructive = FALSE` in `time_to_strat`. This is because last occurrences during a hiatus will appear at the hiatus surface in the stratigraphic domain[^1].

[^1]: What implicit assumption have we made here?
