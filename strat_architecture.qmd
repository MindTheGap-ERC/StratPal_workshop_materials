...
Stratigraphic architecture refers to the three-dimensional geometry of sedimentary strata within a basin, encompassing the distribution of unconformities, facies, systems tracts, and variations in sedimentation rates [@patzkowsky2012stratigraphic; @holland1999].
:::
...
![Production profiles of the three carbonate factories.](figs/platform_production_curve.png){alt="Production profiles of the three carbonate factories."}

The tree factories compete for space and propagate according to a cellular automaton as described in @burgess_2013. The simulation was run for 2 Myr in increments of 1 kyr.
...
```{r strat_architecture_sl_example , echo=FALSE}
library(StratPal)
library(admtools)
...
also called a Wheeler diagram: a plot where the geological time is presented on the vertical axis and the geographic position on the horizontal axis, showing the relative ages and spatial extent of strata [@harrye_wheeler1958]. Wheeler diagrams transform thickness into time, revealing **lacunae**, i.e. gaps representing missing time due to non-deposition (hiatuses) or erosion, that are invisible in conventional sections.
:::

For a comprehensive discussion of what Wheeler diagrams represent, see [@sylvester2024].

![](figs/platform_wheeler_diagram.png)
...
An age-depth model (ADM) is a mapping that estimates the relationship between stratigraphic position (depth) and geological time for a stratigraphic sequence [@lacourse2020; @hohmann_admtools_gchron_2025], allowing paleontologists to assign ages to fossils and sedimentary events based on their position in the rock record [@sadler1981].
:::
...
```{r strat_architecture_adm_example, echo=FALSE}
...
::: callout-important
### Task 1.1: Getting to know age-depth models
...
### Solution Task 1.1: Getting to know age-depth models
...
::: {.callout-note collapse="true"}
### Solution Task 1.2: Incompleteness across the onshore-offshore gradient
...
Incompleteness is highest in the onshore sections (more than 60 %), and drops with distance from shore to values of around 10 % in the proximal slope. Note that this differs from Figure 4 in @hohmann_identification_2024, as we are examining incompleteness on a coarser spatial scale.
...
::: {.callout-note collapse="true"}
### Solution Task 1.3: Hiatus duration and water depth
